...
public abstract class Traversal {
    ...
    protected Traversal(Graph G, Queue<Integer> fringe) {
        _G = G;
        _fringe = fringe;
        _markedverts = new ArrayList<>();
        _visitedverts = new ArrayList<>();
    }
    ...
    public void traverse(Collection<Integer> V0) {
        ...
        while (!_fringe.isEmpty()) {
            int b = _fringe.remove();
            if (!marked(b)) {
                mark(b);
                visit(b);
                _fringe.add(b);
                for (int y : _G.successors(b)) {
                    if (processSuccessor(b, y)) {
                        _fringe.add(y);
                    }
                }
            } else {
                if (shouldPostVisit(b) && !_visitedverts.contains(b)) {
                    postVisit(b);
                    _visitedverts.add(b);
                }
            }
    ...
    /** ArrayList of marked verticies. */
    private ArrayList<Integer> _markedverts;
    /** ArrayList of visited verticies. */
    private ArrayList<Integer> _visitedverts;
